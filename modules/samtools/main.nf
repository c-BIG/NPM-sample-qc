process samtools_stats {
    ...
    seq1=\$(grep "	sequences:" "${sample}.stats" |cut -f3)
    pp_reads=\$(grep "	reads properly paired:" "${sample}.stats" |cut -f3)
    div=\$(awk -v num=\$pp_reads -v denom=\$seq1 'BEGIN { printf ("%.2f", 100 * num / denom) }')
    echo "pct_reads_properly_paired\t"\$div >>"${sample}.samtools.metrics"
