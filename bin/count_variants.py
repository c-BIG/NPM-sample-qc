...
def count_variants(input_vcf, scratch_dir, regions):
    ...
    logging.info("Counting pass_del_het, pass_ins_het...")
    cmd = "bcftools view -H -v indels -f PASS -g het -R %s %s" % (regions, input_vcf)
    # filter dragen annotation of variant records
    cmd += " | sed 's/,<NON_REF>//g'"
    # exclude multi-allelic sites
    cmd += " | awk '$4 !~ /,/ && $5 !~ /,/'"
    # if length(ALT) > length(REF), label as INS_HET
    cmd += " | awk '{ if( length($5) > length($4) ) { print \"INS_HET\" }"
    # if length(ALT) < length(REF), label as DEL_HET
    cmd += " else if ( length($5) < length($4) ) { print \"DEL_HET\" }"
    ...
    logging.info("Counting pass_del_hom, pass_ins_hom...")
    cmd = "bcftools view -H -v indels -f PASS -g hom -R %s %s" % (regions, input_vcf)
    # filter dragen annotation of variant records
    cmd += " | sed 's/,<NON_REF>//g'"
    # exclude multi-allelic sites
    cmd += " | awk '$4 !~ /,/ && $5 !~ /,/'"
    # if length(ALT) > length(REF), label as INS_HOM
    cmd += " | awk '{ if( length($5) > length($4) ) { print \"INS_HOM\" }"
    # if length(ALT) < length(REF), label as DEL_HOM
    cmd += " else if ( length($5) < length($4) ) { print \"DEL_HOM\" }"
    # label everything else as UNK and report its location
    cmd += " else { print \"UNK\" } }'"
    # count by label
    cmd += " | sort | uniq -c"
