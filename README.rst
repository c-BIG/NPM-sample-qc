...
Before running, make sure all the required resources have been downloaded as per the **Resources** section below and build docker image from Dockerfile ``containers/Dockerfile`` or Singularity image from definition file ``containers/singularity.def``.
